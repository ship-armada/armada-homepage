...
export interface ArmadaLogoProps {
  ...
  variant?: 'full' | 'mark'
  /** Mark fill. `brand` — gem gradient. `white` — currentColor (for dark chrome). `deep` — brand deep. */
...
export function ArmadaLogo({ variant = 'full', markTone = 'brand', className }: ArmadaLogoProps) {
  ...
  if (markTone === 'deep') {
    const deepClassName = [styles.fullWrap, styles.fullDeep, className].filter(Boolean).join(' ')
    return (
      <span className={deepClassName} role="img" aria-label="Armada">
        <svg
          ...
          aria-hidden
        >
          <ArmadaMark tone="deep" ids={gradientIds} />
          <path
